...
export async function enviarMensagemDoLojista(
  ...
  try {
    ...
    if (erroRestaurante) {
      console.error(
        `Erro ao buscar dados do restaurante ${restauranteId}: ${erroRestaurante.message}`,
      );
      return {
        ok: false,
        erro:
          'Erro ao buscar dados do restaurante. Tente novamente ou entre em contato com o suporte.',
      };
    }
    ...
    console.error(
      `Erro inesperado em enviarMensagemDoLojista para restaurante ${restauranteId}: ${erro?.message}`,
    );
    return {
      ok: false,
      erro: 'Erro inesperado ao processar a mensagem. Tente novamente ou entre em contato com o suporte.',
    };
  }
}
